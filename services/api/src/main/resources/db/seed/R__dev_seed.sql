...
FROM sc CROSS JOIN (VALUES
    (1000::bigint, '두 수의 합', 'Bronze', 'Bronze V', 1000, 256, 128430, 96322,
     ...
     '첫째 줄에 A+B를 출력한다.'),
    (2231, '분해합', 'Bronze', 'Bronze II', 2000, 192, 74210, 43518,
    ...
    (1932, '정수 삼각형', 'Silver', 'Silver I', 2000, 256, 52104, 33820,
    ...
    (7576, '토마토', 'Gold', 'Gold V', 1000, 256, 89340, 38122,
    ...
    (9019, 'DSLR', 'Gold', 'Gold IV', 6000, 256, 41200, 12844,
     ARRAY['BFS','그래프 탐색'], FALSE,
    ...
    (100001, '정원사의 물결 정렬', 'Silver', 'Silver II', 1000, 256, 842, 517,
    ...
    (100002, '캐시된 미로 탈출', 'Gold', 'Gold III', 2000, 512, 401, 133,
...
) AS p(id, title, difficulty, tier, time_limit_ms, memory_limit_mb,
       submission_count, accepted_count, tags, ai_generated,
...
-- 히든 테스트케이스 — 실제 채점에 쓰이는 데이터(공개 example보다 넓은 범위).
-- api가 이 행들로 MinIO 번들을 만들어 judge에 참조로 넘긴다(claim-check).
-- 현재는 실채점 검증이 가능한 1000(두 수의 합)만 채운다 — 나머지 문제는
-- 채점기 언어 확장·문제별 정답 확보 후 추가(TODO).
INSERT INTO test_case (problem_id, ord, input, output) VALUES
    (1000, 1, '1 2', '3'),
    (1000, 2, '5 4', '9'),
    (1000, 3, '9 9', '18'),
    (1000, 4, '1 1', '2'),
