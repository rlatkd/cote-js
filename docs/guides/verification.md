...
## 현재 절차 (M1 슬라이스: hub(Kotlin) + arena)
...
### 1. hub — 빌드·기동·API 스모크 (:4000)
```bash
cd platform/hub && ./gradlew compileKotlin      # 컴파일 에러 0
...
### 2. 계약 정합 (hub 응답 계약을 바꿨을 때만)
```bash
cd platform/arena && pnpm gen:api          # schema.d.ts 재생성 → git diff 확인 → 커밋
# contract-check.ts가 모델과 어긋나면 아래 3의 next build가 실패한다(의도된 동작)
```

### 3. arena — 빌드 + 실렌더 (:3000)
```bash
cd platform/arena && pnpm build            # next build 통과(계약 체크 포함)
pnpm dev
for p in / /problems /problems/1000 /status; do
  curl -s -o /dev/null -w "$p -> %{http_code}\n" http://localhost:3000$p; done   # 전부 200
# HTML에 hub 시드 데이터 반영 확인 (예: "두 수의 합")
```
- 눈 확인(주요 UI 변경 시): 라이트/다크 토글, split view 리사이즈, Monaco 로딩, 제출 시 stub 결과 표시.

### 4. 품질 게이트
```bash
cd platform/arena && pnpm lint             # ESLint(레이어 의존 규칙 포함)
```

## 추가 예정 (해당 마일스톤 착수 시 이 문서에 절차 추가)

- **hub 테스트 스위트**: Kotest + Testcontainers 도입 시 `./gradlew test`를 게이트에 추가
...
- 2026-07-25 14:07 — hub Kotlin 전환([ADR-0007](../decisions/0007-backend-kotlin-return.md)) 반영: contracts 빌드 단계 삭제 → gradle 컴파일·기동 + OpenAPI/계약 정합(gen:api) 단계로 교체.
