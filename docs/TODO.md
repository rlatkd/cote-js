...
- [ ] ⏸ **problem 생성 트리거 결정**(2026-08-08 미결로 명시, 사용자 지시로 추후 결정) — 관리자 온디맨드 / 재고 기반 자동 / 고정 주기 배치. **지금은 트리거가 아예 없다**(요청 구동인데 요청 발행자가 개발용 probe뿐). 결정 시점 = 검수 큐 가동 후 **반려율이 관측될 때**(자동화 파라미터는 관측에서 나와야 함). 어느 안이든 위 항목의 admin 요청 API가 공통 선결. 근거·비교축은 [engineering-notes](engineering-notes.md) 'problem 생성 트리거'
- [ ] web admin — 검수 큐 UI(M3 검수 게이트)
- [ ] 시드 교체 — 검수 통과 생성 문제로 백준 파생 시드 5문제 대체(ADR-0021 부채)

## 스프린트(종료): 디자인 방향 전환 — 다크 터미널 (2026-08-06) ❌ 폐기 → lab2로 전환

> **2026-08-08 폐기.** 색(본문 `#f1e1b8`와 브랜드 `#d49e68`의 **색상각 13도** → 액센트가 액센트로 안 보임)과 서체(한글 미지원 비트맵이 한 줄 안에서 갈라짐)에서 실패. 아래 미결 항목들은 **더 이상 추적하지 않는다** — 후속은 'lab2' 스프린트로 이어졌고, PIPELINE 패널 미결도 거기서 해소됐다(공정 진행 숫자를 빼고 결과 숫자만 남겨 검증 리포트로 링크).
>
> ⚠️ **`/lab` 코드는 삭제 대상이다.** `3ab67fc`로 커밋돼 있지만 이는 **PC 간 이동 수단**일 뿐 존치 결정이 아니다 — **결정은 문서에 자립해 있다**: 실측값·구조는 [engineering-notes](engineering-notes.md) '디자인 방향 전환', 원리는 [learning-notes](learning-notes.md) '디자인'.
...
- [x] ~~홈 미결: PIPELINE 패널~~ → **lab2에서 해소** — 공정 진행 숫자(생성 12·검증 8)는 사용자 행동으로 안 이어져 홈에서 뺐고, **결과 숫자**(게시·통과율·합의)만 남겨 검증 리포트로 링크
- [x] ~~나머지 4화면 개별 검토~~ → 폐기(lab2에서 전면 재설계)
- [x] ~~`/lab/login` 시안~~ → 폐기. lab2에서는 비로그인 제출 시 **로그인 안내 모달**로 처리

## 현재 스프린트: lab2 — VS Code 레이아웃 차용 (2026-08-08) 🔄

> 랩 1차(`/lab`, 다크 터미널)는 **색·서체 실패로 중단**. 원인 분석과 전환 경위는 [engineering-notes](engineering-notes.md) 'lab2 — VS Code 레이아웃 차용'. `/lab` 코드는 여전히 확정 후 삭제 대상이다.
>
> 원칙: **골격은 VS Code에서, 마감은 우리 것.** 계정 동기화는 api 변경이 필요하므로 **랩에서는 localStorage/sessionStorage만** — 서버 저장은 본 서비스 적용 시(V7).

- [x] **팔레트·서체 확정** (2026-08-08) — VS Code Dark Modern 계열(`#1f1f1f` 에디터 / `#181818` 크롬 / `#cccccc` **무채색 본문**) + 우리 시그널 앰버 `#ff8a00`. 서체는 Pretendard + JetBrains Mono(비트맵 폐기)
- [x] **lab2 1차 골격** (2026-08-08) — 타이틀바·액티비티바·사이드바·탭바·브레드크럼·split 에디터(실 Monaco)·하단 패널·상태바. 5화면
- [x] **UX 흐름 개편** (2026-08-08) — 아래 전부 완료
  - [x] 액티비티 바 = **메뉴 전체**(홈·문제·채점 현황·랭킹·검증·마이페이지 + 하단 설정). 목록 없는 메뉴는 **사이드바를 접고** 화면만 연다
  - [x] 라우팅 = **하이브리드** — URL은 활성 문서 하나, 탭 집합·사이드바 상태는 로컬. 셸을 레이아웃에 둬 라우트 전환에도 리마운트되지 않게
  - [x] 탭을 **진짜로** — 문서를 열면 탭이 생기고 닫힌다. 미리보기 탭 **미채택**. sessionStorage로 세션 한정
  - [x] 검색 **세 자리** — 사이드바 필터(좁히기) / 문서 탭 문제 목록: 표·정렬·다중필터(탐색) / 타이틀바 검색 필드(점프, **실제 동작**)
  - [x] **채점 흐름 실연** — 제출 → 케이스가 0.4초 간격으로 하나씩 채워짐 → 판정. 상태바에도 진행 표시
  - [x] **거짓말 제거** — 하단 패널 "터미널" 탭 삭제, `Ctrl+P` 배지·커맨드 팔레트 어휘 삭제
  - [x] **사이드바 드래그 리사이즈** + 폭을 **비율로** 저장(`clamp(200px, 비율, 480px)`)
  - [x] **코드 초안 보존** — 문제별·**언어별**
  - [x] 내 제출 이력은 **마이페이지에만**. 랭킹은 **별도 메뉴**(M5 전이라 더미)
- [x] **레이아웃 조작을 상태바 우측에 모음** (2026-08-08) — 패널 토글이 계정 옆에 있어 오독됐다. 흩어져 있던 조작(사이드바=액티비티 재클릭·표시 없음 / 패널=타이틀바 / 닫기=헤더 X)을 한곳에. **사이드바 토글이 처음으로 화면에 드러났다**
- [x] **홈 = 문서가 아니라 화면** (2026-08-08) — 서비스 메인 대시보드로 전환(오늘의 문제·내 진행·새 문제·실시간 채점·검증 현황·랭킹). 액티비티 바 최상단, **탭·패널 없이 전체 사용**, 상태바 레이아웃 토글도 감춤. 비로그인은 **자리 유지·내용 교체**
- [x] **액티비티 바를 진짜 링크로** (2026-08-08) — 크롤 경로 확보 + "눌렀는데 반응 없음" 해소 + ⌘+클릭 동작. 선결로 대표 문서 2개 신설(`/status` 채점 현황 표, `/verification` 검증 목록) → 모든 섹션이 "사이드바 목록 + 대표 문서"로 균일. 같은 섹션이면 이동 대신 사이드바 토글. 활성 메뉴는 **URL에서 파생**(`prefs.activity` 상태 제거)
- [x] **SEO 구조** (2026-08-08) — `layout(server)` → `WorkspaceShell(client)` → `page(server, 메타데이터)` → `View(client)`. **client의 children은 server여도 된다**는 점이 열쇠. 문제 페이지는 `generateMetadata`로 제목·지문 도입부·OG, 상태 없는 3화면은 client 미사용, 개인 화면은 `robots: index:false`, `generateStaticParams`로 문제 URL 노출. **실측**: 라우트별 `<title>`·`description` HTML 확인
- [ ] ⏸ **모바일**(사용자 지시로 보류) — "PC 서비스를 만든 뒤에". 데스크톱 레이아웃이 확정돼야 무엇을 접을지가 정해진다
- [ ] ⏸ **색 미결 2건** — ① 브랜드 앰버 `#ff8a00`이 경고색 `#cca700`과 **색상각 17도**라 같은 가족으로 읽힐 위험(랩 1차의 "본문-액센트 13도" 문제와 같은 종류). 대안은 보라 계열이 유일하게 의미론적 색과 안 겹침 ② 전체 색감이 VS Code보다 밝게 느껴지는 문제 — **값은 Dark Modern과 거의 동일**하고 체감 차이는 화면 내용(코드 vs 산문)·글자 크기·헤어라인 양에서 온다. 조절 지점은 배경↓·본문↓·보더↓ 셋
- [ ] 본 서비스 적용 시 — **ADR 2건 필요**: ① 다크 기본 전환은 [ADR-0002](decisions/0002-poc-scope-and-design.md)의 "라이트 기본" 번복 ② VS Code 레이아웃 차용(라이선스 층위: MIT 소스 / CC BY 아이콘 / 상표 불가). 그리고 **sitemap.xml**은 이때 추가(`/lab2` 임시 경로를 사이트맵에 넣을 수 없어 미뤘다)
- [ ] ⚠️ **`/lab`(1차)이 `pnpm lint`를 깨뜨린다** — `react/jsx-no-comment-textnodes` 5건. CI의 web 잡이 lint를 게이트로 쓰므로 `3ab67fc` 이후 CI가 빨간 상태일 수 있다. lab2 확정 후 `/lab` 삭제로 해소 예정
