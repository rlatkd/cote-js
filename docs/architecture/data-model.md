...
> **스키마의 실물은 Flyway 마이그레이션** [`services/api/src/main/resources/db/migration/`](../../services/api/src/main/resources/db/migration/) (V1=스키마, V2=시드) — 이 문서는 관계·소유권·설계 의도를 설명한다. 스키마가 바뀌면 이 문서를 같은 흐름에서 갱신한다(하단 이력에 날짜·시각).

## 소유권 지도 ([ADR-0006](../decisions/0006-service-seams-and-ai-consolidation.md) — 스키마당 단일 작성자)

| 영역 | 주인(유일 작성자) | 상태 |
|---|---|---|
| 코어: problem·example·submission (+ 추후 user·ranking·contest) | **api** (R2DBC + Flyway) | **구현됨** |
