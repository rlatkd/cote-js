...
**레인별 동시성 슬롯(M2, 2026-07-31 적용)**: 위 한계("집어 든 batch가 끝나야 run 실행")를 슬롯으로 개선 — run 2·submit 2·batch 1 세마포어로 폴 배치를 병렬 처리하고, **커밋은 배치 전체 완료 후**(at-least-once 유지). batch는 1슬롯이라 자원을 독식하지 못하고 run·submit은 batch를 기다리지 않는다. 대가: 같은 레인 안 "제출 간" 처리 순서 상실 — 무해(순서가 의미 있는 단위는 제출 1건이고 결과 토픽 키=submission_id로 보존, 제출 사이엔 인과 없음). 슬롯 총합(5)이 동시 채점 컨테이너 상한을 겸한다. 잔여 한계: 폴 배치 단위 대기(다음 폴은 현 배치 완료 후)는 남는다 — 워커 프로세스 수평 확장으로 흡수(컨슈머 그룹 준비됨).
