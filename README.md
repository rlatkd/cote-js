...
```
              외부 공개 데이터셋 (내부 이용 한정 — ADR-0021)
           (few-shot 스타일 참고 · 유사도 코퍼스 전용)
        ※ 게시 문제는 100% 자체 생산, 기존 사이트 크롤링 배제
                                |
                                v
...
- **데이터 라이선스 (2026-08-01 확정 — [ADR-0021](docs/decisions/0021-data-licensing.md))**: 게시 문제는 100% 자체 생산(AI 생성+검수·수작성)으로 하여 재배포 리스크를 구조적으로 제거. 외부 공개 데이터셋은 내부 용도(생성 참고·유사도 코퍼스)만, 채택 시 개별 라이선스 실확인. 기존 사이트 크롤링·공개셋 게시는 배제. 잔여 부채: 백준 파생 시드 5문제를 M3에서 생성 문제로 교체.
...
```
참고 데이터셋(내부 한정) → 생성(LLM API) → 유사도 검증(임베딩/pgvector)
